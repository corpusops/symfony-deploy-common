...
NO_START=${NO_START-}
SYMFONY_CONF_PREFIX="${SYMFONY_CONF_PREFIX:-"SYMFONY__"}"
...
export CRON_LOGS_DIR="${CRON_LOGS_DIR:-$SRC_DIR/private/logs}"
export FPM_LOGS_DIR="${FPM_LOGS_DIR:-$SRC_DIR/private/logs}"
...
configure() {
    ...
    if [ -f /code/app/.front_index.frep ]; then
        FRONT_INDEX_FILE="${FRONT_INDEX_FILE:-"$SRC_DIR/public/front/dist/index.html"}"
        log "regenerate ${FRONT_INDEX_FILE}"
        frep "$SRC_DIR/.front_index.frep:${FRONT_INDEX_FILE}" --overwrite
    ...
    if [ -e $SRC_DIR/var/nginxwebroot ] && [[ -z ${NO_COLLECT_STATIC} ]]; then
        ...
        rsync -a --delete --exclude files/ $SRC_DIR/public/ $SRC_DIR/var/nginxwebroot/ \
...
do_fg() {
    ...
    ( cd $PROJECT_DIR && exec gosu $APP_USER php bin/console --no-interaction server:run $SYMFONY_LISTEN )
}
